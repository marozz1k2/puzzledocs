...
* **Модель GPT:** Выпадающий список для выбора языковой модели, которая будет обрабатывать сообщения. Выбор модели влияет на качество ответов и скорость реакции. Рекомендуется использовать актуальные версии (например, указанную на скриншоте «GPT-5.4») для наиболее точного следования инструкциям.
...
* **Стоимость:** работа функции включена в тариф Бизнес.
* **Стоимость:** каждый ответ «секретаря» списывает указанное для выбранной модели количество 💠. Подробнее в статье [«Модели и стоимость»](../../info/ai-modeli-i-stoimost.md).
