...
Эта инструкция описывает, как настроить в боте учёт и списание 💠 с баланса пользователей.
...
2. После успешного выполнения, система PxAI автоматически вызывает в вашем боте заранее определенную команду-триггер (например, `gpt_image_done`).
...
1.  **Создайте переменную для баланса:**
    ...
    * Создайте новую персональную переменную (например, `req_exp`) с типом «Число» и значением по умолчанию `0`. В ней будет храниться баланс 💠 каждого пользователя.
...
**Готово!** Теперь после генерации через модель `Джипити 5 нано` у пользователя будет списываться 💠1, после чего система проверит остаток баланса.&#x20;
