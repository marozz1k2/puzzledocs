...
{% hint style="info" %}
Эта инструкция показывает готовый сценарий на Suno. Сейчас в PuzzleAI также доступны Producer, QW Music и FlowMusic, а в Suno появились новые операции редактирования. Краткий обзор смотрите на странице [«Музыка: модели и операции»](../../info/muzykalnye-modeli-i-operacii.md), рабочие поля — в [документации Трекера](https://docs.pxsto.re/tracker/treker-zaprosy/music).

Сценарий можно упростить: ID готового трека Suno записывает в переменную сам (по умолчанию `suno_music_id`, второй вариант — `suno_music_id_2`), кавер и продление принимают исходное аудио ссылкой или файлом из формы ввода без отдельной загрузки, а имена полей запроса читаются без учёта регистра.
{% endhint %}
...
2. **Генерация в расширенном режиме.** Полная кастомизация трека, включая загрузку своего текста, выбор стиля, модели Suno, голоса вокалиста и других параметров.
