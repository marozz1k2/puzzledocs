...
Бесплатная текстовая модель на базе GPT-5 Nano для быстрых ответов и черновиков.
...
```jsonc
{
  ...
  "model": "gpt_free", // обязательно: точный ключ модели.
...
| `model` | string | Да | Точный ключ вызываемой модели: `gpt_free`. |
