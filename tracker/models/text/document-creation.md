...
- Для новых сценариев используйте `template`, а `style` оставляйте для совместимости.
- Если документ должен отправляться в конкретный чат/топик, передавайте `chat` и `topic` явно.
- Если нужно обрабатывать сбои отдельным сценарием, заранее задайте `error_command`.
