...
```jsonc
{
  ...
  "model": "vision", // обязательно: точный ключ модели.
  "prompt": "{{prompt}}", // обязательно: текст задачи для модели.
  "role": "[текст роли]", // необязательно: роль, стиль или ограничения для модели.
  "images": "https://example.com/photo.jpg,https://example.com/reference-2.jpg" // обязательно: ссылки, file_id или переменные с изображениями через запятую, без массива `[]`.
}
```
...
| `model` | string | Да | Точный ключ вызываемой модели: `vision`. |
...
| `images` | string | Да | Одна или несколько ссылок, file_id или переменных с изображениями через запятую, без массива `[]`. |
