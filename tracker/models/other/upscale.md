...
```jsonc
{
  ...
  "model": "upscale", // обязательно: точный ключ модели.
  ...
  "params": { // необязательно: вложенные настройки конкретной модели.
    "scale_factor": "2x", // необязательно: масштаб увеличения.
    "upscale_preset": "standard" // необязательно: пресет обработки.
  }
}
```
...
| `model` | string | Да | Точный ключ вызываемой модели: `upscale`. |
| `prompt` | string | Нет | описание желаемой обработки. Алиасы поддерживаются для совместимости: `text`. Для новых запросов используйте `prompt`. |
| `images` | string | Да | входное изображение для апскейла. |
| `params` | object | Нет | параметры обработки в зависимости от сценария. |
