...
Видео-генерация из текста и референсов в быстром режиме. В текущей конфигурации используется `google/veo-3.1-fast`.
...
```jsonc
{
  ...
  "model": "veo_fast", // обязательно: точный ключ модели.
  "prompt": "{{prompt}}", // обязательно: текст задачи для модели.
  "role": "[текст роли]", // необязательно: роль, стиль или ограничения для модели.
  "images": "https://example.com/reference1.jpg,https://example.com/reference2.jpg", // необязательно: ссылки, file_id или переменные с изображениями через запятую, без массива `[]`.
  "params": { // необязательно: вложенные настройки конкретной модели.
    "resolution": "1080p" // необязательно: желаемое разрешение результата.
  }
}
```
...
| `model` | string | Да | Точный ключ вызываемой модели: `veo_fast`. |
| `prompt` | string | Да | описание видео. |
| `images` | string | Нет | до 2 изображений. |
| `params.resolution` | string | Нет | по умолчанию `1080p`. |
