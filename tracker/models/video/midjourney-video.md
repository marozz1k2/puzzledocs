...
Видео-модель Midjourney Video для генерации роликов по тексту и референсам.
...
Обязательная база для любого запроса: `bot`, `token`, `user`, `model`. Дальше добавьте содержимое задачи: `prompt`, `images` или `video_url`.
...
```jsonc
{
  ...
  "model": "midjourney_video", // обязательно: точный ключ модели.
...
| `model` | string | Да | Точный ключ вызываемой модели: `midjourney_video`. |
