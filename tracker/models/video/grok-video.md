...
```jsonc
{
  ...
  "model": "grok_video", // обязательно: точный ключ модели.
  ...
  "params": { // необязательно: вложенные настройки конкретной модели.
    "duration": 6 // необязательно: длительность результата.
  }
}
```
...
| `model` | string | Да | Точный ключ вызываемой модели: `grok_video`. |
| `prompt` | string | Да | описание видео. |
| `images` | string | Нет | до 1 изображения. |
| `params.duration` | number | Нет | по умолчанию `6`. |
