...
Видео-генерация из текста и референсов в качественном режиме. В текущей конфигурации используется `google/veo-3.1-quality`.
...
```jsonc
{
  ...
  "model": "veo", // обязательно: точный ключ модели.
...
| `model` | string | Да | Точный ключ вызываемой модели: `veo`. |
