...
```jsonc
{
  ...
  "model": "higgsfield_video", // обязательно: точный ключ модели.
  "prompt": "{{prompt}}", // обязательно: текст задачи для модели.
  "role": "[текст роли]", // необязательно: роль, стиль или ограничения для модели.
  "images": "https://example.com/reference1.jpg,https://example.com/reference-2.jpg", // необязательно: ссылки, file_id или переменные с изображениями через запятую, без массива `[]`.
...
| `model` | string | Да | Точный ключ вызываемой модели: `higgsfield_video`. |
| `prompt` | string | Да | описание видео. |
| `images` | string | Нет | до 2 изображений. |
| `params.duration` | number | Нет | по умолчанию `5`. |
| `params.sound` | boolean | Нет | по умолчанию `false`. |
