...
```jsonc
{
  ...
  "model": "kling_image", // обязательно: точный ключ модели.
  ...
  "params": { // необязательно: вложенные настройки конкретной модели.
    "resolution": "1k", // необязательно: желаемое разрешение результата.
...
| `model` | string | Да | Точный ключ вызываемой модели: `kling_image`. |
| `prompt` | string | Да | описание изображения. |
| `images` | string | Нет | входные изображения/референсы. |
| `params.resolution` | string | Нет | `1k` или `2k`, по умолчанию `1k`. |
| `params.aspect_ratio` | string | Нет | по умолчанию `auto` (для `kling-o1-image` без `auto`, дефолт `16:9`). |
