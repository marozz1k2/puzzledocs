...
```jsonc
{
  ...
  "model": "midjourney", // обязательно: точный ключ модели.
  ...
  "params": { // необязательно: вложенные настройки конкретной модели.
    ...
    "stylize": 100 // необязательно: степень стилизации.
  }
}
```
...
| `model` | string | Да | Точный ключ вызываемой модели: `midjourney`. |
| `prompt` | string | Да | описание изображения. |
| `images` | string | Нет | входные изображения/референсы. |
| `params` | object | Нет | дополнительные параметры рендера. |
